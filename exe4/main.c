...
#define CHECK_MS       5      // leitura rápida do ADC (↓ latência)

// Metade do período (intervalo entre alternâncias)
#define TOGGLE_MS_Z1   150    // período total de 300 ms
#define TOGGLE_MS_Z2   250    // período total de 500 ms
...
static bool blink_cb(repeating_timer_t *t) {
    ...
    return true; // segue repetindo
}

static inline void start_blink_timer(int interval_ms) {
